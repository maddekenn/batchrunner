...
public class RecordTypePGroupIdsModifierTest {
    ...
    public void setUp(){
        ...
        modifier = RecordTypePGroupIdsModifier.usingURLAndHttpHandlerFactory(url, httpHandlerFactory);
    }
    ...
    public void testModifyPGroupsDoesNotExist(){
        ...
        assertEquals(httpHandlerForRecordTypeRead.urlString, "http://someTestUrl/recordType/myRecordType");

        HttpHandlerSpy httpHandlerForPGroupRead = httpHandlerFactory.httpHandlerSpies.get(1);
        assertEquals(httpHandlerForPGroupRead.urlString, "http://someTestUrl/presentationGroup/myRecordTypePGroup");
        ...
        assertEquals(httpHandlerForPFormGroupRead.urlString, "http://someTestUrl/presentationGroup/myRecordTypeFormPGroup");
        assertEquals(httpHandlerForPFormGroupRead.requestMethod, "GET");



        String readPresentationGroup = httpHandlerForPFormGroupRead.getResponseText();

        ClientDataRecord pGroupAsDataRecord = getPGroupJsonAsClientDataRecord(readPresentationGroup);
        ...
        String pGroupJsonWithoutRecordInfo = getDataGroupAsJson(pGroupAsDataGroup);
        ...
        assertEquals(httpHandlerForPGroupCreate.requestProperties.get("Accept"), "application/vnd.uub.record+json");
        assertEquals(httpHandlerForPGroupCreate.requestProperties.get("Content-Type"), "application/vnd.uub.record+json");
        assertEquals(httpHandlerForPGroupCreate.outputString, pGroupJsonWithoutRecordInfo);
    ...
    private String getDataGroupAsJson(ClientDataGroup dataGroup) {
        ...
        DataToJsonConverter forClientDataElement = jsonConverterFactory.createForClientDataElement(factory, dataGroup);
